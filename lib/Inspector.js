                ...
                if(scopeHashes[scope.$id] !== hc) {
                    ...
                    angular.forEach(members, function(value,key) {
                        if(scope.hasOwnProperty(key) &&
                            typeof isolatedBindings[key] === 'undefined' &&
                            Inspector.hasPrimitivePropInScopeChain(Inspector.getProto(scope),key) === true) {
                            shadowed[key] = true;
                        }
                    });
                    if(!isEmptyObj(shadowed)) {
                        console.warn('..SCOPE: ' + scope.$id);
                        console.warn('....members shadowed: ');
                        console.warn('....' + Object.keys(shadowed));
                    }
                    var possiblyShadowed = {};
                    angular.forEach(inheritedMembers, function(value,key) {
                        if(!shadowed[key]) {
                            possiblyShadowed[key] = true;
                        }
                    });
                    if(!isEmptyObj(possiblyShadowed)) {
                        console.warn('..SCOPE: ' + scope.$id);
                        console.warn('....members could be shadowed: ');
                        console.warn('....' + Object.keys(possiblyShadowed));
                    }
