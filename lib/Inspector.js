    ...
    angular.module('scope-aware', [])
        .service('Inspector', ['$rootScope', function ($rootScope) {
            ...
            var guessAngularDir = function(element) {
                if(element) {
                    var possibleDir = ['ng-repeat', 'ng-include', 'ng-transclude', 'ng-view'];
            ...
            var hasPrimitivePropInScopeChain = function(anyScope,prop) {
                if(anyScope === null) {
                    return false;
                }
                if(anyScope.hasOwnProperty(prop)) {
                    return !(angular.isObject(anyScope[prop]) || angular.isFunction(anyScope[prop]));
                } else {
                    return hasPrimitivePropInScopeChain.call(null,anyScope.$parent,prop);
                }
            };
            ...
            var traverse = function (anyScope, visitor, lvl) {
                ...
                angular.forEach(pieces, function (value) {
                    ...
                    if (inherited && inherited[1]) {
                        ...
                        if(hasPrimitivePropInScopeChain(anyScope.$parent,inherited[1])) {
                            if(marker.length > 0) {
                                marker += '; ';
                            }
                            if(anyScope.hasOwnProperty(inherited[1])) {
                                marker += ' shadowed !!!';
                            } else {
                                marker += ' shadowing danger';
                            }
                        }
                    }
                    if(marker !== '') {
            ...
            var toShadow = function(util, customEqualityTesters) {
                return {
                    compare: function(actual, expected) {
                        ...
                        var shadowed = [];
                        expected.forEach(function(member) {
                            if(actual.hasOwnProperty(member) && Inspector.hasPrimitivePropInScopeChain(actual.$parent,member) === true) {
